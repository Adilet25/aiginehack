      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
        <div className="glow-card rounded-3xl border border-white/10 bg-white/5 p-8">
          <div className="mx-auto flex max-w-xl flex-col items-center text-center">
            ...
            >
              <span className={`text-xl font-black tracking-wide text-white ${isOpening ? 'animate-pulse' : ''}`}>
                {isOpening ? 'OPENING...' : 'CHÜKO BOX'}
              </span>
            </div>

            <p className="max-w-md text-sm leading-7 text-white/65">
              Открывай коробку и получай случайный культурный артефакт. Если предмет уже есть в коллекции, ты получишь компенсацию очками.
            </p>
        ...
        <div className="space-y-4">
          <div className="glow-card rounded-3xl border border-white/10 bg-white/5 p-6">
            <h3 className="text-lg font-bold text-white">Drop Chances</h3>
            <div className="mt-4 space-y-3">
              {chances.map((chance) => (
                ...
                >
                  <span className="text-sm text-white/75">{chance.label}</span>
                  <span className="text-sm font-bold text-emerald-300">{chance.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glow-card min-h-[240px] rounded-3xl border border-white/10 bg-white/5 p-6">
            <h3 className="text-lg font-bold text-white">Latest Reward</h3>
              ...
              <div className="mt-4 space-y-4">
                <div className="rounded-3xl border border-emerald-400/20 bg-gradient-to-br from-emerald-500/10 to-cyan-500/10 p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h4 className="text-xl font-black text-white">{result.name}</h4>
                      <p className="mt-2 text-sm text-white/65">{result.description}</p>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                      ...
                      <p className="mt-1 text-lg font-bold text-white">{result.points}</p>
                    </div>

                    <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                      <p className="text-xs text-white/45">Income</p>
                      <p className="mt-1 text-lg font-bold text-white">+{result.income}</p>
                    </div>
                  </div>
                </div>
